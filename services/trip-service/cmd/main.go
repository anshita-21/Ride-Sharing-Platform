...
func main() {
	...
	// Initialize Repository (MongoDB or In-Memory fallback)
	var tripRepo domain.TripRepository
	mongoClient, err := db.NewMongoClient(ctx, db.NewMongoDefaultConfig())
	if err != nil {
		log.Printf("MongoDB unavailable (%v). Falling back to in-memory repository.", err)
	...
	svc := service.NewService(tripRepo)
